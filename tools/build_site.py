...
<style>
  ...
  * { box-sizing:border-box; }
  ...
  .tab.active { background:var(--accent); border-color:var(--accent); color:#04122c; font-weight:600; }
    .tabs.focused .tab { display:none; }
    #back-markets { display:none; margin-bottom:12px; }
    #back-markets.visible { display:inline-block; }
...
<div class="wrap">
  ...
  <div class="tabs">
    ...
    <button class="tab" data-t="common">Shared Positions</button>
  ...
  <div class="section" id="s-news">
    <div class="controls">
      <input type="search" id="q-news" placeholder="Search headline or fund…">
      <select id="f-source"><option value="">All sources</option></select>
      <button class="btn" id="only-bbg">Bloomberg only</button>
      <span class="count" id="c-news"></span>
    </div>
    <div style="overflow:auto; max-height:74vh;"><table id="t-news"></table></div>
  </div>
  ...
  <div class="section" id="s-stock">
    <button class="backbtn" id="back-stock">← Shared positions</button>
    <h2 id="stock-name"></h2>
...
function renderMarkets() {
  ...
  makeTable(document.getElementById('t-indices'), marketCols, m.indices||[]);
  makeTable(document.getElementById('t-fx'), marketCols, m.currencies||[]);
  makeTable(document.getElementById('t-crypto'), cryptoCols, m.crypto||[]);
}
...
function showMainView(target) {
  document.querySelectorAll('.tab').forEach(x=>x.classList.remove('active'));
  document.querySelectorAll('.section').forEach(x=>x.classList.remove('active'));
  document.querySelector(`.tab[data-t="${target}"]`).classList.add('active');
  document.getElementById('s-'+target).classList.add('active');
  document.querySelector('.tabs').classList.toggle('focused', target !== 'markets');
  document.getElementById('back-markets').classList.toggle('visible', target !== 'markets');
  window.scrollTo(0,0);
}
...
document.getElementById('only-bbg').addEventListener('click', e => {
  ...
  const enabled = source.value !== 'Bloomberg.com';
  source.value = enabled ? 'Bloomberg.com' : '';
...
document.getElementById('f-country').addEventListener('change',()=>{ renderFunds(); renderNews(); renderActions(); renderSignals(); renderAnalysts(); renderCommon(); renderBanner(); renderMarkets(); });
...
async function loadData(initial) {
  try {
    ...
    populateCountries();
    renderActions(); renderNews(); renderCommon(); renderFunds();
