...
<div class="wrap">
  ...
  <div class="tabs">
    ...
    <button class="tab" data-t="news">News (by date)</button>
    <button class="tab" data-t="signals">News Signals</button>
...
function renderCards() {
  const totalBuys = DATA.funds.reduce((s,f)=>s+f.buys,0);
  const totalSells = DATA.funds.reduce((s,f)=>s+f.sells,0);
  const aum = DATA.funds.reduce((s,f)=>s+(f.total_value||0),0);
  const cards = [
    ['Funds', DATA.funds.length], ['Total Buys', fmtNum(totalBuys)],
    ['Total Sells', fmtNum(totalSells)], ['Common Holdings', fmtNum(DATA.common.length)],
    ['Live Prices', fmtNum(DATA.quotes_count||0)], ['News Articles', fmtNum(DATA.news_feed_count||0)],
    ['Combined AUM', fmtUsd(aum)],
  ];
...
document.getElementById('only-bbg').addEventListener('click',(e)=>{
  const sel=document.getElementById('f-source');
  const on = sel.value!=='Bloomberg.com';
  sel.value = on ? 'Bloomberg.com' : '';
